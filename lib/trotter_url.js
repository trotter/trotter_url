...
Trotter.Url = function (url) {
    ...
    */
    ...
    if ((new RegExp("^" + pattern.splitAbsolute + "$")).test(url)) {
        split = url.replace(new RegExp(pattern.splitAbsolute), "$1 $2 $3 $4 $5 $6 $7 $8 $9").split(" ");
        ...
        this.host = split[3];
        this.port = split[4];
        this.registry = split[5];
        this.path = split[6];
        this.queryString = split[7];
        this.fragment = split[8];
    } else if ((new RegExp("^" + pattern.splitRelative + "$")).test(url)) {
        split = url.replace(new RegExp(pattern.splitRelative), "$1 $2 $3 $4 $5 $6 $7 $8").split(" ");

        this.userInfo = split[0];
        this.host = split[1];
        this.port = split[2];
        this.registry = split[3];
        this.relSegment = split[4];
        this.path = split[5];
        this.queryString = split[6];
        this.fragment = split[7];
    };
};
